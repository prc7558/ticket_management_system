...
/**
 * Admin Dashboard with 4 panels:
 * 1. All Tickets (searchable, sortable)
 * 2. Priority Assign
 * 3. Status Updater
 * 4. Reports
 */
public class AdminDashboard extends JFrame {
    ...
    private void initUI() {
        ...
        JLabel titleLbl = ThemeConstants.createLabel("🛡 Admin Dashboard", ThemeConstants.FONT_SUBTITLE, ThemeConstants.TEXT_PRIMARY);
        JLabel adminLbl = ThemeConstants.createLabel("Admin: " + currentAdmin.getName(), ThemeConstants.FONT_BODY, ThemeConstants.TEXT_SECONDARY);
        ...
        tabbedPane.addTab("⚡ Priority Assign", createPriorityPanel());
        tabbedPane.addTab("🔄 Status Updater", createStatusPanel());
    ...
    private JPanel createAllTicketsPanel() {
        ...
        allTicketsTable.getColumnModel().getColumn(4).setCellRenderer(new DefaultTableCellRenderer() {
            ...
            public Component getTableCellRendererComponent(JTable t, Object v, boolean sel, boolean foc, int r, int c) {
                ...
                if (!sel && v != null) {
                    comp.setForeground(ThemeConstants.getPriorityColor(v.toString()));
        ...
        searchBtn.addActionListener(e -> {
            ...
            if (q.isEmpty()) refreshAllTickets();
            else searchTickets(q);
        });
        ...
        searchBar.add(ThemeConstants.createLabel("All Tickets", ThemeConstants.FONT_SUBTITLE, ThemeConstants.TEXT_PRIMARY));
    ...
    private JPanel createPriorityPanel() {
        ...
        JLabel heading = ThemeConstants.createLabel("Assign Priority", ThemeConstants.FONT_SUBTITLE, ThemeConstants.TEXT_PRIMARY);
        heading.setAlignmentX(Component.LEFT_ALIGNMENT);

        JLabel tidLbl = ThemeConstants.createLabel("Ticket ID", ThemeConstants.FONT_HEADING, ThemeConstants.TEXT_SECONDARY);
        tidLbl.setAlignmentX(Component.LEFT_ALIGNMENT);
        JTextField tidField = ThemeConstants.createStyledTextField(10);
        tidField.setMaximumSize(new Dimension(Integer.MAX_VALUE, 40));
        tidField.setAlignmentX(Component.LEFT_ALIGNMENT);

        JLabel priLbl = ThemeConstants.createLabel("Priority", ThemeConstants.FONT_HEADING, ThemeConstants.TEXT_SECONDARY);
        ...
        priBox.setMaximumSize(new Dimension(Integer.MAX_VALUE, 40));
        ...
        assignBtn.setMaximumSize(new Dimension(Integer.MAX_VALUE, 42));
        ...
        card.add(tidLbl); card.add(Box.createVerticalStrut(4)); card.add(tidField);
        card.add(Box.createVerticalStrut(14));
        card.add(priLbl); card.add(Box.createVerticalStrut(4)); card.add(priBox);
        card.add(Box.createVerticalStrut(20));
        card.add(assignBtn);
    ...
    private JPanel createStatusPanel() {
        ...
        JLabel heading = ThemeConstants.createLabel("Update Ticket Status", ThemeConstants.FONT_SUBTITLE, ThemeConstants.TEXT_PRIMARY);
        heading.setAlignmentX(Component.LEFT_ALIGNMENT);
        ...
        JLabel curLbl = ThemeConstants.createLabel("Current Status: —", ThemeConstants.FONT_BODY, ThemeConstants.TEXT_SECONDARY);
        curLbl.setAlignmentX(Component.LEFT_ALIGNMENT);

        // Load button to fetch current
        JButton loadBtn = ThemeConstants.createStyledButton("Load", ThemeConstants.ACCENT_BLUE);
        loadBtn.setPreferredSize(new Dimension(80, 34));
        loadBtn.setAlignmentX(Component.LEFT_ALIGNMENT);

        JLabel newLbl = ThemeConstants.createLabel("New Status", ThemeConstants.FONT_HEADING, ThemeConstants.TEXT_SECONDARY);
        ...
        statusBox.setMaximumSize(new Dimension(Integer.MAX_VALUE, 40));
        ...
        loadBtn.addActionListener(e -> {
            try {
                ...
                if (t != null) {
                    curLbl.setText("Current Status: " + t.getStatus().name() + "  |  Priority: " + t.getPriority());
        ...
        updateBtn.setMaximumSize(new Dimension(Integer.MAX_VALUE, 42));
        ...
        updateBtn.addActionListener(e -> {
            try {
                ...
                if (service.updateStatus(id, ns, currentAdmin.getId())) {
                    ...
                    curLbl.setText("Current Status: " + ns.name());
        ...
        loadRow.setMaximumSize(new Dimension(Integer.MAX_VALUE, 40));
        loadRow.add(loadBtn);
        ...
        card.add(curLbl);
        card.add(Box.createVerticalStrut(14));
        card.add(newLbl); card.add(Box.createVerticalStrut(4)); card.add(statusBox);
        card.add(Box.createVerticalStrut(20));
        card.add(updateBtn);
        card.add(Box.createVerticalStrut(8));
        card.add(resultLbl);
    ...
    // ════════════════════════════════════════════
    // TAB 4: Smart Matcher (from campus-issue CIRS)
    // ════════════════════════════════════════════
    private JPanel createSmartMatcherPanel() {
        ...
        topBar.add(ThemeConstants.createLabel("Smart Matcher — Related Tickets", ThemeConstants.FONT_SUBTITLE, ThemeConstants.TEXT_PRIMARY));

        JButton scanBtn = ThemeConstants.createStyledButton("Scan All", new Color(138, 99, 255));
                ...
                if (matches.isEmpty()) {
                    ...
                    for (int i = 0; i < matches.size(); i++) {
                        ...
                        sb.append("  ──── Match #").append(i + 1)
                          .append(" (Similarity: ").append(String.format("%.0f%%", m.getScore() * 100)).append(") ────\n");
    ...
    private JPanel createReportsPanel() {
        ...
        // Summary text
        JTextArea summaryArea = ThemeConstants.createStyledTextArea(10, 40);
        ...
        Runnable loadReports = () -> {
            ...
            SwingUtilities.invokeLater(() -> {
                ...
                sb.append("  On Track:      ").append(total - overdue - res - closed).append("\n\n");
        ...
        panel.add(summaryArea.getParent() instanceof JScrollPane ? summaryScroll : summaryArea, BorderLayout.CENTER);
